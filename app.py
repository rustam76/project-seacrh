...
def detail(id):
    ...
    cursor.execute("SELECT title, abstract, link FROM tb_dokument WHERE id = %s", (id,))
    ...
    try:
        ...
        for _ in range(50):  # Batas hingga 50 hasil
            try:
                ...
                cursor.execute("SELECT COUNT(*) FROM tb_dokument WHERE title = %s", (title,))
                if cursor.fetchone()[0] == 0:  # Jika tidak ada duplikasi, masukkan data
                    cursor.execute("""
